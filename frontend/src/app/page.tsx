...
export default function Home() {
  ...
  const fetchResources = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}/api/gcp/resources`,
      );
  ...
  const fetchActiveResources = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}/api/gcp/resources/active`,
      );
      const data = await res.json();
      if (data && !data.error) {
      ...
      interval = setInterval(async () => {
        try {
          ...
          if (data.status === "Completed") {
            setStatus(
              "Authentication successful! You can now proceed to discovery.",
            );
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-blue-100">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight">
                GeoShield <span className="text-blue-600">Control Plane</span>
              </h1>
          ...
          <div className="flex items-center gap-2 bg-blue-50 px-4 py-2 rounded-full border border-blue-100">
            <div
              className={`w-2 h-2 rounded-full ${loading ? "bg-blue-500 animate-ping" : "bg-green-500"}`}
            />
          ...
          <div className="flex flex-col md:flex-row md:items-end gap-6">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
              {steps.map((step, idx) => (
                ...
                >
                  <div
                    className={`flex flex-col items-center px-3 py-1 rounded-lg transition-all ${viewMode === step.id ? "bg-blue-50 border border-blue-200" : "opacity-40 hover:opacity-80"}`}
                  >
                    <span
                      className={`text-[10px] font-black uppercase ${viewMode === step.id ? "text-blue-600" : "text-slate-400"}`}
                    >
                      Step {idx + 1}
                    </span>
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-3 bg-blue-600 text-white px-5 py-3 rounded-xl shadow-md">
            ...
            <span className="text-sm font-bold tracking-wide">
              {status || "Ready to begin landing zone discovery."}
            </span>
          </div>
            ...
            {viewMode === "auth" && (
              <div className="bg-white p-12 rounded-2xl shadow-sm border border-orange-100 flex flex-col items-center text-center space-y-4 animate-in fade-in zoom-in duration-300">
                ...
                <p className="max-w-md text-slate-500 text-sm leading-relaxed">
                  Initializing secure session with Google Cloud. We use service
                  account impersonation to ensure high security without storing
                  keys.
                </p>
              ...
              <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-300">
                <div className="flex items-center justify-between px-2">
                  ...
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                    {Object.keys(resources).length} Service Types Found
                  </span>
                </div>
              ...
              <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-300">
                ...
                {Object.entries(activeResources).map(([service, rows]) => {
                  ...
                  return (
                    <div
                      key={service}
                      className="bg-white rounded-2xl shadow-sm border border-emerald-100 overflow-hidden transition-all duration-200"
                    >
                      <div
              ...
              <div className="bg-white p-20 rounded-2xl border-2 border-dashed border-blue-100 flex flex-col items-center justify-center text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-400 max-w-xs mx-auto mt-1">
                    Begin by authenticating with your GCP project to scan for
                    resources.
                  </p>
                </div>
              </div>
